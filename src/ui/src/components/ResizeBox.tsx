...
export default function ResizeBox(props) {
	...
	onMount(() => {
		setBoxSize((vertical() ? resizeBox.clientHeight : resizeBox.clientWidth) / 2);
		createResizeObserver({
			refs: [resizeBox],
			onResize: (size) => {
				setBoxSize(Math.min(boxSize(), (vertical() ? size.height : size.width) - 8));
			},
		});
	});
	...
	const drag = (e) => {
		...
		const newSize = (vertical() ? e.clientY : e.clientX) - offset - (vertical() ? resizeBar.clientHeight : resizeBar.clientWidth);
	...
	return (
		<div
			ref={resizeBox}
			class={[props.class, styles.resizeBox, vertical() ? styles.resizeBoxVertical : styles.resizeBoxHorizontal].join(" ")}
			style={{
